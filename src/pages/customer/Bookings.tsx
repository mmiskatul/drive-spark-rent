      ...
      <div className="mt-6 space-y-3">
        {list.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-border bg-card p-16 text-center">
            ...
            <Button asChild className="mt-6 rounded-full"><Link to="/cars">Browse cars</Link></Button>
          </div>
        ) : list.map((b) => (
          <Link key={b.id} to={`/customer/bookings/${b.id}`} className="block rounded-2xl border border-border bg-card hover:shadow-soft transition-shadow">
