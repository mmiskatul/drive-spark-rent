    ...
    <AuthLayout title={sent ? "Check your inbox" : "Forgot password?"} subtitle={sent ? "We sent a reset link to your email." : "We'll send you a reset link."}>
      {!sent ? (
        <form onSubmit={(e) => { e.preventDefault(); setSent(true); }} className="space-y-4">
          ...
          <p className="text-center text-sm text-muted-foreground">Remembered? <Link to="/login" className="text-primary font-semibold hover:underline">Back to sign in</Link></p>
        </form>
      ) : (
        <div className="rounded-2xl border border-border bg-card p-8 text-center">
          ...
          <Button asChild variant="outline" className="mt-6 rounded-full"><Link to="/login">Back to sign in</Link></Button>
        </div>
      )}
    </AuthLayout>
  );
