...
export default function Login() {
  ...
  return (
    <AuthLayout title="Welcome back" subtitle="Sign in to your DriveNow account">
      <form onSubmit={(e) => { e.preventDefault(); toast.success("Signed in"); nav("/customer"); }} className="space-y-4">
        <div><Label>Email</Label><Input className="mt-1.5 h-11" type="email" placeholder="you@example.com" /></div>
        <div>
          <div className="flex justify-between items-center"><Label>Password</Label><Link to="/forgot-password" className="text-xs text-primary hover:underline">Forgot?</Link></div>
        ...
        <p className="text-center text-sm text-muted-foreground">Don't have an account? <Link to="/register" className="text-primary font-semibold hover:underline">Sign up</Link></p>
      </form>
    </AuthLayout>
  );
}
